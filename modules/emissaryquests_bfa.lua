...
-----------------------------------------------------------
local name = "Emissary Quests (BfA)";
...
if not rawget(L,name) then
	L[name] = name;
end
if not rawget(L,"Emissary Quests-ShortCut-BfA") then
	L["Emissary Quests-ShortCut-BfA"] = "EQA";
end
...
local function updateBroker()
	...
	if UnitLevel("player")<120 then
		obj.text = ns.profile[name].shortTitle and L["Emissary Quests-ShortCut-BfA"] or L[name];
		return
	end
...
local function createTooltip(tt)
	...
	local l=tt:AddLine();
	tt:SetCell(l,1,C("dkyellow",L[name]),tt:GetHeaderFont(),"LEFT",0);
...
function module.options()
	return {
		broker = {
			shortTitle={ type="toggle", order=1, name="Show shorter title", desc=L["Display '%s' instead of '%s' on chars under level 110 on broker button"]:format(L["Emissary Quests-ShortCut-BfA"],L["Emissary Quests (BfA)"]) }
		},
		tooltip = {
			showAllFactions=1,
			showRealmNames=2,
